...
function create_artifacts_for_pv_az_file_volume () {
    ...
    create_azure_volume_secret \
      "${deployment_json}" \
      "${sa_name}" \
      "${volume_name}" \
      "${volume_prefix}"
    create_k8s_persistent_volume \
      "${deployment_json}" \
      "${volume_name}" \
      "${volume_prefix}" \
      "${volume_quota}" \
...
function get_volume_metadata_by_name () {
...
function iterate_az_file_volumes () {
    ...
    volumes="$(jq -r -e '.target.paas.storage.azure_files[].name' <<< "${cluster_config_json}")"
    for volume_name in ${volumes}; do
        local volume_metadata
        volume_metadata="$(get_volume_metadata_by_name "${cluster_config_json}" "${volume_name}")"
